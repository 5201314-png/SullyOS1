...
const Settings: React.FC = () => {
    ...
    const [showTtsPreModal, setShowTtsPreModal] = useState(false);
    const [ttsPreEnabled, setTtsPreEnabled] = useState(() => localStorage.getItem('ttsPreEnabled') === 'true');
    const [ttsPreUrl, setTtsPreUrl] = useState(() => localStorage.getItem('ttsPreUrl') || 'https://api.kourichat.com/v1');
    const [ttsPreKey, setTtsPreKey] = useState(() => localStorage.getItem('ttsPreKey') || '');
    const [ttsPreModel, setTtsPreModel] = useState(() => localStorage.getItem('ttsPreModel') || 'deepseek-v4-flash');
    const [ttsPrePrompt, setTtsPrePrompt] = useState(() => localStorage.getItem('ttsPrePrompt') || `你是一个声优导演。你的任务是为即将被 TTS 引擎朗读的文本添加语气标签和停顿标记，让合成的语音听起来像一个真人在自然说话——有气息、有情绪、有节奏，而不是机器在念稿。
...
8. 直接输出处理后的文本，不要添加任何解释。`);
    ...
    // 这是保存按钮按下去时执行的动作
    const handleSaveTtsPre = () => {
        localStorage.setItem('ttsPreEnabled', String(ttsPreEnabled));
        localStorage.setItem('ttsPreUrl', ttsPreUrl);
        localStorage.setItem('ttsPreKey', ttsPreKey);
        localStorage.setItem('ttsPreModel', ttsPreModel);
        localStorage.setItem('ttsPrePrompt', ttsPrePrompt);
        setShowTtsPreModal(false);
        alert('语音合成配置已保存！'); // 弹出个成功提示
    };
    ...
    return (
        <div className="h-full w-full bg-slate-50/50 flex flex-col font-light relative">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6 no-scrollbar pb-20">
                ...
                <section className="bg-white/80 rounded-3xl p-5 shadow-sm border border-white/50">
                    ...
                    <div className="space-y-4">
                        ...
                        <button
                            ...
                            className={`w-full py-2.5 rounded-2xl font-bold text-sm border mt-2 active:scale-95 transition-all ${testingApi || !localUrl.trim() || !localKey.trim() || !localModel.trim()
                        ...
                        {testApiResult && (
                            <div className={`mt-2 text-xs px-3 py-2 rounded-xl ${testApiResult.startsWith('✅') ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'
                                }`}>
                                {testApiResult}
                            </div>
                        )}
                ...
                <section className="bg-white/80 rounded-3xl p-5 shadow-sm border border-white/50">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        {/* 语音合成预处理入口 */}
                        <div className="bg-gradient-to-r from-fuchsia-50 to-purple-50 border border-fuchsia-100 rounded-2xl p-4 mb-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <div className="text-sm font-bold text-fuchsia-800">语音合成配置 (TTS 预处理)</div><div className="text-xs text-fuchsia-600/70 mt-0.5">为语音添加情感、呼吸、停顿等自然标签</div>
                                </div>
                                <button
                                    onClick={() => setShowTtsPreModal(true)}
                                    className="px-4 py-1.5 bg-white text-fuchsia-600 text-xs font-bold rounded-xl shadow-sm hover:shadow active:scale-95 transition-all"
                                >
                                    配置
                                </button>
                            </div>
          ...
          out owner via /user and auto-create a private 'sully-backup' repo. */}
            ...
            <Modal isOpen={showTtsPreModal} title="语音合成配置" onClose={() => setShowTtsPreModal(false)}>
                <div className="p-4 space-y-4">
                    {/* 开关区域 */}
                    <div className="flex items-center justify-between bg-purple-50/50 p-3 rounded-xl border border-purple-100/50">
                        <div>
                            <div className="text-sm font-bold text-slate-700">AI 语气预处理</div>
                            <div className="text-[10px] text-slate-500 mt-1">开启后，发送到 TTS 之前先用独立 AI 为文本添加语气词标签，让朗读更自然。</div>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                            <input
                                type="checkbox"
                                className="sr-only peer"
                                checked={ttsPreEnabled}
                                onChange={(e) => setTtsPreEnabled(e.target.checked)}
                            />
                    ...
                    <div className="space-y-3">
                        <div>
                            <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">API BASE URL</label>
                            <input
                                type="text"
                                value={ttsPreUrl}
                                onChange={(e) => setTtsPreUrl(e.target.value)}
                                placeholder="https://api.kourichat.com/v1"
                                className="w-full bg-slate-50 border border-slate-200/60 rounded-xl px-3 py-2 text-xs focus:bg-white transition-all font-mono"
                            />
                        </div>
                        <div>
                            ...
                            <input
                                type="password"
                                value={ttsPreKey}
                                onChange={(e) => setTtsPreKey(e.target.value)}
                                placeholder="sk-..."
                                className="w-full bg-slate-50 border border-slate-200/60 rounded-xl px-3 py-2 text-xs focus:bg-white transition-all font-mono"
                            />
                        </div>
                        <div>
                            <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">模型 推荐 flash/turbo</label>
                            <input
                                type="text"
                                value={ttsPreModel}
                                onChange={(e) => setTtsPreModel(e.target.value)}
                        ...
                        <div>
                            <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1 mt-3">预处理提示词</label>
                            <textarea
                                rows={6}
                                value={ttsPrePrompt}
                                onChange={(e) => setTtsPrePrompt(e.target.value)}
                                className="w-full bg-slate-50 border border-slate-200/60 rounded-xl px-3 py-2 text-[10px] focus:bg-white transition-all leading-relaxed resize-none"
                            />
                        </div>
                    </div>

                    <button
                        onClick={handleSaveTtsPre}
                        className="w-full py-3 bg-gradient-to-r from-pink-300 to-purple-400 text-white shadow-md text-sm font-bold rounded-xl active:scale-95 transition-transform mt-4"
                    >
                        保存配置
                    </button>
                </div>
